...
void loop() {
  ...
  ang_xy = atan2(accel_y, accel_x) * rad2deg;

  if(ang_xz < 0) ang_xz += 360;
  if(ang_yz < 0) ang_yz += 360;
  if(ang_xy < 0) ang_xy += 360;

  mag = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  bx = accel_x/mag;
  by = accel_y/mag;
  bz = accel_z/mag;

  q0=cos(ang_xz/2);
  q1=sin(ang_xz/2)*cos(bx); 
  q2=sin(ang_yz/2)*cos(by); 
  q3=sin(ang_xy/2)*cos(bz); 

  roll= atan2((q0*q1 + q2*q3),(0.5 - (q1*q1 + q2*q2)));
  pitch = asin(2*(q0*q2 - q1*q3));
  yaw = atan2((q0*q3 + q1*q2),(0.5 - (q2*q2 + q3*q3)));
